...
def inference(net, img: np.ndarray, th: float = 0.7, input_size=(320, 240)):
    global image_std, center_variance, size_variance
    witdh = input_size[0]
    height = input_size[1]
    priors = define_img_size(input_size)
    rect = cv2.resize(img, (witdh, height))
    ...
    boxes, _, _ = predict(img.shape[1], img.shape[0], scores, boxes, th)
    return boxes
